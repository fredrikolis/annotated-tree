// Concern: the MCP stdio server — the crate's one async surface — exposing the map, graph and strict builders as tools | Non-concern: any builder logic | IO: (Cli) -> JSON-RPC over stdio + exit code
//
