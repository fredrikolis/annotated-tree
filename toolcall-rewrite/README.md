...
src/model.rs  # Concern: the canonical in-memory map and every filesystem read behind it — a sorted tree of each fil …
src/render/json.rs  # Concern: serializes the canonical map as the versioned, machine-readable JSON contract | Non-concern …
src/render/md.rs  # Concern: formats the canonical map as human-facing Markdown | Non-concern: filesystem reads or machi …
